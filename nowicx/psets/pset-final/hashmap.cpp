...
/// of data across the hash table and thereby prevent clustering
/// 
/// Hash Table
...
// calls unordered_map erase() and returns its return value 1 or 0.
bool erase(unordered_map<string, int>& ht, string key) {
	DPRINT(cout << ">erase: " << key << endl;);

	bool ans = ht.erase(key);
...
void show_range(unordered_map<string, int>& ht, vector<int>& bucket_idx, size_t from, size_t to) {
	...
	for (int i = from; i < to ;i++) {
		...
		for (auto local_it = ht.begin(bucket_idx[i]); local_it != ht.end(bucket_idx[i]); ++local_it)
			cout << " " << local_it->first << ":" << local_it->second;
		cout << endl;
	}

}
void show(unordered_map<string, int>& ht, bool show_empty = false, int show_n = 0) {
	if (show_n <= 0 || show_n > ht.bucket_count()) show_n = (int) ht.bucket_count();
...
int main(int argc, char* argv[]) {
	...
	do {
		...
		case 'f':  // find
			...
			{
				if (ht.find(key) != ht.end()) {
					cout << "\t Found!: " << ht.find(key)->first << " : " << ht.find(key)->second << endl;
				}
				else {
