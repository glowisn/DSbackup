...
void rehash(Hash*& ht) {
    DPRINT(cout << "rehash: " << ht.tablesize << endl);
...
void rehash(Hash*& ht, int n) {  // n is a user specified size
    ...
    if (n >= old_size) {
        ...
        for (int i = 0; i < tempsize; i++) {
            wordcount iwc = temp.back();
            for (int j = 0;j < iwc.second; j++) {
                insert(ht, iwc.first);
            }
            temp.pop_back();
        }
...
bool insert(Hash*& ht, string key) {
    ...
    // rehash if necessary
    if (loadfactor(ht) >= ht->threshold) rehash(ht);
...
bool erase(Hash*& ht, string key) {
    ...
    for (it = ht->hashtable[index].begin(); it != ht->hashtable[index].end(); ++it) {
        if (it->first == key) {
            ht->hashtable[index].remove(make_pair(it->first, it->second));
            ht->nelements--;
            return true;
        }
    }
