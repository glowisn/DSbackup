...
bool heapOrderedAt(heap p, int k) {
	if (k >= p->N / 2) return true; // check it upto the last second internal node.

	int left  = 2 * k;
	int right = 2 * k + 1;

	if (p->comp(p, k, left))  return false;
	if (p->comp(p, k, right)) return false;
...
void growN(heap p, int count, bool heapOrdered) {
	...
	for (int i = 0; i < count; i++) {
		(*insertFunc)(p, arr[i]);
		DPRINT(cout << heapprint(p, TREE_MODE) << endl;)
	}
	delete[] arr;
	DPRINT(cout << "<growN" << endl;);
}
