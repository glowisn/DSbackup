...
bool findPathBack(tree root, tree x, vector<int>& path) {
	...
	if (findPathBack(root->left, x, path) || findPathBack(root->right, x, path) == true) {
		path.push_back(root->key);
		return true;
	}
...
bool findPath(tree node, tree x, vector<int>& path) {
	...
	bool pathLeft = findPath(node->left, x, path);
	bool pathRight = findPath(node->right, x, path);

	if (pathLeft || pathRight == true) return true;
	else path.pop_back();
	...
	// found the key: delete the node now 
		...
		if (node->left == nullptr && node->right == nullptr) {
			...
			if (height(node->left) > height(node->right)) {
				//left is tall, call pred
				tree prede = pred(node);
				node->key = prede->key;
				node->left = trim(node->left, node->key);
			}
			else {
				//right is tall, call succ
				tree succe = succ(node);
				node->key = succe->key;
...
void levelorder(tree root, vector<int>& vec) {
	...
	while (!empty(que)) {
		tree fnode = que.front();
		vec.push_back(fnode->key);
		if (fnode->left != nullptr) que.push(fnode->left);
		if (fnode->right != nullptr) que.push(fnode->right);
		que.pop();
	}
...
tree LCA_BT(tree root, tree p, tree q) {  // recursive solution
	...
	// base vase 2: itself is p or q return that node
	if (root == p || root == q) return root;

	// recursive case
	tree lnode = LCA_BT(root->left, p, q);
	tree rnode = LCA_BT(root->right, p, q);
...
int LCA_BTiteration(tree root, tree p, tree q) {  
	...
	int n;
	int maxiterate = min(pathp.size(), pathq.size());
	for (n = 0; n < maxiterate; n++) {
		if (pathp[n] != pathq[n])return pathp[n - 1];
	}
...
tree LCA(tree root, tree p, tree q) {  // recursive solution
	...
	if (root->key < p->key && root->key < q->key) {
		//if p and q are in the right subtree(large)
		return LCA(root->right, p, q);
	}
	else if (root->key > p->key && root->key > q->key) {
		//if p and q are in the left subtree(small)
		return LCA(root->left, p, q);
	}
	else {
		//if p and q are in the different branch
		return root;
	}
...
int LCAiteration(tree root, tree p, tree q) {  // iteration solution
	...
	tree curr = root;
	while ((p->key > curr->key && q->key > curr->key) || (p->key < curr->key && q->key < curr->key)) {
		if (curr->key < p->key && curr->key < q->key) {
			//if p and q are in the right subtree(large)
			curr = curr->right;
		}
		else if (curr->key > p->key && curr->key > q->key) {
			//if p and q are in the left subtree(small)
			curr = curr->left;
		}
	}
...
tree putBST(vector<int> v, tree root, int& index) {
	if (root == nullptr) return root;
	
	root->left = putBST(v, root->left, index);
	root->key = v[index++];
	root->right = putBST(v, root->right, index);
