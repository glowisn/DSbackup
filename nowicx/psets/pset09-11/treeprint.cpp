...
// put nodes in vector that is in a same height
void levelorder_height(tree root, vector<int>& vec, int level) {
	if (root == nullptr) return;
	if (level == 1)
		//put root in vector
		vec.push_back(root->key);
	else {
		levelorder_height(root->left, vec, level - 1);
		levelorder_height(root->right, vec, level - 1);
	}
