...
pNode half(pList p) {  
	...
	int size_2 = size(p) / 2;
	pNode c = p->head;
	for (int i = 0; i < size_2; i++, c = c->next);
...
pNode half(pList p) { 
	...
	pNode rabbit = begin(p) , turtle = begin(p);
...
#if 0
////////// this part shown below is added for padagogical reason //////////////
// returns n_th node in the list - a helper function
...
void randomize(pList p) {
	...
	//shuffle the array
	for (i = N - 1; i > 0; i--) {
		int j = rand_extended(i) % i;
		swap(arr[i], arr[j]);
	}

	//change value with array O(n)
	curr = begin(p);
	i = 0;
	while (curr != end(p)) {
		curr->data = arr[i++];
...
void push_backN(pList p, int N) {
	...
	//cout << "your code here \n";
	for (int i = 0; i < N; i++) {
		push_back(p, rand_extended(range) % range);
	}
...
void unique(pList p) {
	...
	for (pNode x = begin(p); x != end(p); x = x->next)
		if (x->data == x->prev->data) erase(p, x->prev);
...
bool sorted(pList p, int(*comp)(int a, int b)) {
	...
	pNode curr = begin(p);
	while (curr != last(p)) {
		if (0 > (*comp)(curr->next->data, curr->data)) {
			return false;
		}
		curr = curr->next;
	}
...
void push_sorted(pList p, int val) {
	...
	*/
	...
	if (sorted(p, ascending)) {
		while (curr != end(p) && 0 > more(curr->data, val)) {
			curr = curr->next;
		}
		insert(curr, val);
	}
	else {
		while (curr != end(p) && 0 > ::less(curr->data, val)) {
			curr = curr->next;
		}
		insert(curr, val);
	}
